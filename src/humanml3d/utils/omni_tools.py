...
def create_list_chunks(list_, group_size, overlap_size, cut_smaller_batches=True):
    if cut_smaller_batches:
        return [
            ...
            if len(list_[i : i + group_size]) == group_size
        ]
    else:
        return [
            list_[i : i + group_size]
...
class log2file:
    def __init__(
        self, logpath=None, prefix="", auto_newline=True, write2file_only=False
    ):
    ...
    for tIdx in range(T):
        ...
        for mIdx in range(N):
            ...
            for jIdx in range(n_joints):
                cur_axisangle.append(
                    cv2.Rodrigues(
                        matrots[mIdx, tIdx, jIdx : jIdx + 1, :].reshape(3, 3)
                    )[0].T
                )
