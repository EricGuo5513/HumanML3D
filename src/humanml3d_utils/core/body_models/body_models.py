...
class BodyModel(nn.Module):
    def __init__(
        ...
        if self.use_dmpl and self.model_type in [
            "smplx",
            "mano",
            "animal_horse",
            "animal_dog",
        ...
        self.comp_register(
            "init_v_template",
            torch.tensor(smpl_dict["v_template"][None], dtype=dtype),
        ...
        if self.model_type == "smplx":
            ...
            self.comp_register(
                "exprdirs",
                torch.tensor(exprdirs, dtype=dtype),
                persistent=persistant_buffer,
            )

            expression = torch.tensor(np.zeros((1, num_expressions)), dtype=dtype)
            self.comp_register(
                "init_expression", expression, persistent=persistant_buffer
            )
        ...
        if self.use_dmpl:
            ...
            self.comp_register(
                "dmpldirs",
                torch.tensor(dmpldirs, dtype=dtype),
                persistent=persistant_buffer,
            )
        ...
        self.comp_register(
            "weights", torch.tensor(weights, dtype=dtype), persistent=persistant_buffer
        )

        self.comp_register(
            "init_trans", torch.zeros((1, 3), dtype=dtype), persistent=persistant_buffer
        )
        ...
        if self.model_type in ["smpl", "smplh", "smplx"]:
            self.comp_register(
                "init_pose_body",
                torch.zeros((1, 63), dtype=dtype),
                persistent=persistant_buffer,
            )
        elif self.model_type == "animal_horse":
            self.comp_register(
                "init_pose_body",
                torch.zeros((1, 105), dtype=dtype),
                persistent=persistant_buffer,
            )
        elif self.model_type == "animal_dog":
            self.comp_register(
                "init_pose_body",
                torch.zeros((1, 102), dtype=dtype),
                persistent=persistant_buffer,
            )
        ...
        if self.model_type in ["smpl"]:
            self.comp_register(
                "init_pose_hand",
                torch.zeros((1, 1 * 3 * 2), dtype=dtype),
                persistent=persistant_buffer,
            )
        elif self.model_type in ["smplh", "smplx"]:
            self.comp_register(
                "init_pose_hand",
                torch.zeros((1, 15 * 3 * 2), dtype=dtype),
                persistent=persistant_buffer,
            )
        elif self.model_type in ["mano"]:
            self.comp_register(
                "init_pose_hand",
                torch.zeros((1, 15 * 3), dtype=dtype),
                persistent=persistant_buffer,
            )
        ...
        self.comp_register(
            "init_betas",
            torch.zeros((1, num_betas), dtype=dtype),
            persistent=persistant_buffer,
        )
